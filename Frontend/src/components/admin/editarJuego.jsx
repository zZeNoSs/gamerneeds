...
const EditarJuego = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      ...
      try {
        ...
        const desarrolladorId = juego.desarrolladores_ids ? juego.desarrolladores_ids.split(',')[0] : '';
        const editorId = juego.editores_ids ? juego.editores_ids.split(',')[0] : '';       
        const generosIds = juego.generos_ids 
          ? juego.generos_ids.split(',').map(id => parseInt(id.trim()))
          : [];        
        setFormData({
          titulo: juego.titulo || '',
          descripcion: juego.descripcion || '',
          precio: juego.precio || '',
          fecha_lanzamiento: juego.fecha_lanzamiento ? juego.fecha_lanzamiento.split('T')[0] : '',
          clasificacion_edad: juego.clasificacion_edad ? String(juego.clasificacion_edad) : '',
          url_trailer: juego.url_trailer || '',
          url_portada: juego.url_portada || '',
          desarrollador_iddesarrollador: desarrolladorId,
          editor_ideditor: editorId,
          generos: generosIds
        });        
      } catch (error) {
  ...
  const validateField = (name, value) => {
    switch (name) {
      ...
      case 'clasificacion_edad':
        ...
        if (!edadesValidas.includes(String(value))) return 'Selecciona una clasificación válida';
        return '';
  ...
  const validateForm = () => {
    ...
    Object.keys(formData).forEach(key => {
      if (key !== 'url_portada' && key !== 'url_trailer') {
  ...
  if (loading && !formData.titulo) {
    return (
      <div className="p-8 flex justify-center items-center">
        <div className="text-white text-xl">Cargando datos del juego...</div>
      </div>
    );
  }
  return (
    <div className="p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6" noValidate>
        ...
        <div className="space-y-2">
          <label htmlFor="fecha_lanzamiento" className="block text-sm font-medium text-gray-400">Fecha de Lanzamiento *</label>
          <input
            type="date"
        ...
        <div className="space-y-2">
          ...
          {!selectedImage && formData.url_portada && (
            <div className="text-sm text-blue-400 mt-1">
              📷 Imagen actual: {formData.url_portada}
            </div>
          )}
          <div className="text-sm text-gray-400 mt-1">
            Formatos: JPEG, JPG, PNG, WEBP. Tamaño máximo: 10MB. Si no seleccionas ninguna, se mantendrá la imagen actual.
          </div>
        </div>
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-400">Desarrollador *</label>
          <select
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-400">Editor *</label>
          <select
        ...
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-400">Géneros * (máximo 5)</label>
